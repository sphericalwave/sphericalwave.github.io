---
layout: page
title: Customer Surveys
permalink: /surveys/
description: "sphericalWave builds embedded customer surveys for small businesses — real feedback, structured data, no third-party tool required."
---

<div class="sw-home">

  <section class="sw-home__hero sw-enter">
    <span class="sw-home__chip">
      <span class="sw-home__dot"></span>
      <span class="sw-home__label">Surveys</span>
    </span>
    <h1 class="sw-home__title">Feedback that feeds your workflow.</h1>
    <p class="sw-home__lead">Embedded customer surveys built into your app or site — responses land in a structured dashboard, not a spreadsheet.</p>
    <div class="sw-home__cta">
      <a href="https://cal.com/sphericalwave" class="sw-home__btn sw-home__btn--primary" rel="noopener">Book a call</a>
      <a href="/services/" class="sw-home__btn sw-home__btn--ghost">All services</a>
    </div>
    <div class="sw-home__ornament" aria-hidden="true">
      <span class="sw-home__orn-grow"></span>
      <span class="sw-home__orn-mid"></span>
      <span class="sw-home__orn-grow"></span>
    </div>
  </section>

  <hr class="sw-home__wave-line">

  <div class="sw-home__bento sw-enter-2">
    <div class="sw-home__glass sw-home__panel sw-home__panel--accent">
      <span class="sw-home__label">Examples</span>
      <p class="sw-home__panel-title">Survey examples coming soon.</p>
      <p>Real client survey flows and dashboards will be shown here.</p>
    </div>
    <div class="sw-home__glass sw-home__panel sw-home__panel--center">
      <span class="sw-home__label">Start here</span>
      <p class="sw-home__panel-title">Scope your survey</p>
      <a href="mailto:hello@sphericalwave.com" class="sw-home__btn sw-home__btn--ghost">Email me</a>
    </div>
  </div>

  <hr class="sw-home__wave-line">

  <section class="sw-home__cta-band sw-enter-2">
    <h2>Know what your customers actually think.</h2>
    <p>I build the survey, wire up the data, and hand you something you can act on.</p>
    <div class="sw-home__cta">
      <a href="https://cal.com/sphericalwave" class="sw-home__btn sw-home__btn--primary" rel="noopener">Book a call</a>
      <a href="mailto:hello@sphericalwave.com" class="sw-home__btn sw-home__btn--ghost">Email me</a>
    </div>
  </section>

</div>
